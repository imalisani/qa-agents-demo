Feature: Reject invalid refund requests without changing financial state
  Background:
...
Feature: Customer-visible refund totals remain consistent
  Background:
...
Feature: Accessible refund interactions
...
Feature: Domain-level financial invariants
...
Feature: Publish only current verified results
